#!/bin/bash
# WaveProxy v1.0 (Python版) - gh-pages 分支专用安装脚本
...
echo "🌊 Installing WaveProxy Python v1.0 (gh-pages branch)..."
...
curl -L https://raw.githubusercontent.com/Sha0huaZhang/waveproxy/gh-pages/bin/waveproxy.py -o ~/.local/waveproxy/bin/waveproxy.py
...
curl -L https://raw.githubusercontent.com/Sha0huaZhang/waveproxy/gh-pages/bin/proxywrap.sh -o ~/.local/waveproxy/bin/proxywrap.sh
...
curl -L https://raw.githubusercontent.com/Sha0huaZhang/waveproxy/gh-pages/bin/proxydeploy.sh -o ~/.local/waveproxy/bin/proxydeploy.sh
...
if [ ! -f ~/.local/waveproxy/proxydeploy@default.txt ]; then
    ...
    curl -L https://raw.githubusercontent.com/Sha0huaZhang/waveproxy/gh-pages/config/proxydeploy@default.txt -o ~/.local/waveproxy/proxydeploy@default.txt
    echo -e "${GREEN}🌊 Default config file generated successfully${NC}"
fi

# 6. 下载命令参考文件
echo "🌊 Downloading COMMAND_REFERENCE.txt..."
curl -L https://raw.githubusercontent.com/Sha0huaZhang/waveproxy/gh-pages/COMMAND_REFERENCE.txt -o ~/.local/waveproxy/COMMAND_REFERENCE.txt
...
echo ""
echo -e "${GREEN}🌊 WaveProxy Python v1.0 installation complete!${NC}"
echo ""
